...
const EVENT_PAGE_CATEGORY_LABELS: Record<string, { label: string; emoji: string }> = {
  bar:        { label: 'Bar',        emoji: '🍻' },
...
function eventPageFallback(type: string, id: string, noindex: boolean): string {
...
function eventPageRender(data: Record<string, unknown>, id: string, pageUrl: string): string {
  ...
  const image = (data.promotionPhotoUrl as string) || (data.photoUrl as string) || 'https://alertoo.com.br/feature-graphic-1024x500.png';
