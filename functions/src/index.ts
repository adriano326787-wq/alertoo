...
export const verifyMPPayment = onCall(
  ...
  async (request) => {
    ...
    if (!preferenceId) throw new HttpsError('invalid-argument', 'preferenceId obrigatório.');
    ...
    if (results.length === 0) return { status: 'pending' };
    ...
    if (mpStatus === 'approved') {
      ...
      if (refUid !== uid) throw new HttpsError('permission-denied', 'Pagamento não pertence a este usuário.');

      const pkg = CREDIT_PACKAGES[refPackageId];
      if (!pkg) throw new HttpsError('invalid-argument', 'Pacote do pagamento inválido.');
