...
export const onBetaTesterAdded = onDocumentCreated(
  { document: 'beta_testers/{entryId}', region: 'us-central1', secrets: [RESEND_API_KEY] },
  async (event) => {
    ...
    const pendingSnap = await db.collection('beta_testers').where('invited', '==', true).count().get();
